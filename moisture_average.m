...
data = thingSpeakRead(channelID, 'Fields', 2, 'ReadKey', readAPIKey, 'DateRange', [startDate, endDate]);
...
disp(['The average moisture level is ', num2str(avgMoisture), '%.']);
disp(['The standard deviation of moisture is ', num2str(stdMoisture), '%.']);
disp(['The average loss of moisture over ', num2str(numDays), ' days is ', num2str(avgMoistureLoss), '% per day.']);